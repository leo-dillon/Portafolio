...
export const Presentation = () => {
    return (
        <section className="h relative w-full max-w-9/10 min-h-150 sm:min-h-screen pt-12 pb-20
            flex flex-col items-center gap-2 justify-center border-b border-teal-800/50 
            text-center text-stone-700
            dark:text-stone-200
        ">
            <h1 className="
            w-full md:maw-w-9/10 aparecer-L2
            text-5xl text-wrap font-bold px-8 py-5 text-teal-950 dark:text-gray-100 sm:mb-6
            sm:rounded-full 
            md:text-8xl 
            lg:text-9xl 
            ">
                Leonardo <span className="inline-block text-4xl md:text-5xl opacity-30 aparecer-L ">Nahuel</span> <span className="inline-block text-4xl md:text-5xl opacity-30 aparecer-L">Jeannoteguy</span>  Dillon 
            </h1>
            <p className="sm:mt-12 text-2xl md:text-4xl text-blue-950/70 font-bold
                dark:text-teal-300 aparecer-L
            ">
                Programador Full-Stack
            </p>
            <p className="mt-4 text-xl md:text-2xl text-stone-600 
            dark:opacity-60 dark:text-stone-300 aparecer-L">
                Desarrollador de aplicaciones accesibles, atractivas y modernas
            </p>
            <div 
            className="absolute bottom-4 p-4 py-2 flex flex-row gap-4 bg-stone-200 shadow-sm shadow-teal-800
                dark:bg-stone-950 dark:shadow-teal-500
                rounded-full
            ">
