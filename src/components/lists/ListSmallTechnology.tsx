...
export const ListSmallTechnology = ({ technologies }: Props ) => {
    ...
    return (
        <ul className="w-full max-w-9/10 mx-auto relative flex flex-row flex-wrap gap-4">
            {
                ( arrayTechs )
                ? arrayTechs.map((tech) => {
                    return (
                        <li aria-label={tech.name} key={tech.name}
                            ...
                            className="flex flex-col items-center duration-150 cursor-pointer
                            hover:scale-110
                            active:scale-90
                        ">
                            <img className="w-6 h-6" src={tech.img} title={tech.name} />
                        </li>        
                    )
                })
                : ""
